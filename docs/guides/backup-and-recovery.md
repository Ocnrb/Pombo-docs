...
The file is encrypted under your **account password** (verified before export) — without it, the file is useless to whoever finds it. Treat it with the same care as the key itself.
...
Paid channels never re-distribute past keys, and the announcements that anchor older ones expire with the channel's retention. For history in those channels, your backup is the only recovery path — see [Encryption](../concepts/encryption.md#rotation-and-history).
:::
